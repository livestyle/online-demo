...
export default class WidgetOverlay {
	constructor(editor) {
		...
		var self = this;
		this._onWidgetDispose = function() {
		...
		delegate(elem, 'click', '.ls-widget', function(evt) {
			var widget = null;
	...
	add(widget, line, ch) {
		...
		widget.on('dispose', this._onWidgetDispose);
	}
	...
	remove(widget) {
		widget.detach();
		widget.off('dispose', this._onWidgetDispose);
